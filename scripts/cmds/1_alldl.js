...
module.exports = {
  ...
  onChat: async function ({ api, event }) {
    ...
    if (this.checkLink(event.body)) {
      ...
      api.setMessageReaction("�9�0", event.messageID, (err) => {}, true);
    }
  },
  downLoad: function (url, api, event) {
    const time = Date.now();
    const path = __dirname + `/tmp/${time}.mp4`;
  ...
  downloadInstagram: async function (url, api, event, path) {
    try {
      ...
      const messageBody = `�7�3 �9�3 Download Url: ${shortUrl}`;
    ...
    try {
      ...
      if (res.success && res.download && res.download.length > 0) {
        ...
        response.data.on('end', async () => {
          ...
          const messageBody = `Autodownload`;
